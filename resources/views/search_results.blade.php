...
<head>
    ...
    <style>
        ...
        .sidebar nav ul li a:hover { /* Menghilangkan background-color pada hover */
            color: #173F63;
            /* background-color: #e0f7fa; */ /* DIHAPUS */
        }
        .sidebar nav ul li a.active { /* Menghilangkan background-color pada active */
            color: #173F63;
            /* background-color: #e0f7fa; */ /* DIHAPUS */
        }
        .sidebar nav ul li a svg { /* SVG fill dengan currentColor */
            margin-right: 15px;
            width: 20px;
            height: 20px;
            color: #888;
        ...
        .video-card img, /* Tambahkan video ke selector ini */
        .video-card video { /* Pastikan gaya ini berlaku untuk elemen video juga */
            width: 100%;
...
<body>
    ...
    <div class="main-content-search">
        ...
        @if($videos->isEmpty())
            <p class="no-results">No video found for "{{ $query ?? '' }}".</p>
        @else
            <div class="search-results-grid">
                @foreach($videos as $video)
                    <div class="video-card">
                        {{-- LOGIKA BARU UNTUK MENAMPILKAN THUMBNAIL/VIDEO/GAMBAR --}}
                        @if(Str::startsWith($video['mimeType'] ?? '', 'video/'))
                            {{-- Menggunakan elemen <video> dengan atribut poster --}}
                            <video controls muted loop poster="{{ asset('images/default-thumbnail.png') }}"> {{-- Pastikan default-thumbnail.png ada --}}
                                <source src="{{ Storage::url($video['pathFile']) }}" type="{{ $video['mimeType'] }}">
                                Your browser does not support the video tag.
                            </video>
                        @elseif(Str::startsWith($video['mimeType'] ?? '', 'image/'))
                            {{-- Jika itu gambar, tampilkan sebagai <img> langsung dari pathFile --}}
                            <img src="{{ Storage::url($video['pathFile']) }}" alt="{{ $video['description'] ?? 'Post Image' }}">
                        @else
                            {{-- Fallback jika bukan video/gambar atau mimeType tidak terdefinisi --}}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const clearOverlaySearchInputButton = searchFormOverlay.querySelector('button');
