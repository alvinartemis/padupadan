        ...
        <div style="display: flex; justify-content: space-between; align-items: center; padding-bottom: 20px; border-bottom: 1px solid #eee; margin-bottom: 20px;">
            <a href="{{ route('chat.profileuser', $user) }}" style="display: flex; align-items: center; text-decoration: none; color: inherit;">
                ...
                <div>
                    ...
                    <p style="margin: 5px 0 0 0; color: #777; font-size: 0.9rem;">{{ '@' . $user->username }}</p>
                </div>
            </a>
            <button onclick="window.location.href='{{ route('chat.indexstylist') }}'" style="background: none; border: none; color: #999; font-size: 1.5rem; cursor: pointer;">
                &times;
            </button>
        </div>
        <div style="flex-grow: 1; overflow-y: auto; padding-bottom: 20px;" id="message-container">
            @include('chat.listmessagestylist', ['messages' => $messages])
        </div>
        <form action="{{ route('chat.sendMessageStylist', $user) }}" method="POST" enctype="multipart/form-data" style="display: flex; align-items: center; padding-top: 20px; border-top: 1px solid #eee;">
            @csrf
